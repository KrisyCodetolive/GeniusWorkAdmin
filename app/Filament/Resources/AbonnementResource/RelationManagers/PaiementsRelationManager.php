            ...
            ->headerActions([
                Tables\Actions\CreateAction::make()
                    ->mutateFormDataUsing(function (array $data) {
                        // Ajouter l'ID de l'entreprise et de l'initiateur
                        $data['entreprise_id'] = $this->ownerRecord->entreprise_id;
                        $data['initiateur_id'] = auth()->id();
                        
                        // Si une facturation est associée, l'ajouter
                        $facturation = $this->ownerRecord->facturations()->latest()->first();
                        if ($facturation) {
                            $data['facturation_id'] = $facturation->id;
                        }
                        
                        return $data;
                    })
                    ->after(function (Paiement $record) {
                        // Si le paiement est complété, mettre à jour le statut de l'abonnement
                        if ($record->statut === 'complete') {
                            $this->ownerRecord->update([
                                'statut' => 'actif',
                            ]);
                            
                            // Si une facturation est associée, la marquer comme payée
                            if ($record->facturation) {
                                $record->facturation->update([
