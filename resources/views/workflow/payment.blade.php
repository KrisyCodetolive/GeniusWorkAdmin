...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('paymentForm', () => ({
            selectedGateway: '{{ $selectedGateway }}',
            selectedMethod: '{{ $selectedMethod }}',
            facturationId: '{{ $facturation_id ?? null }}',
            isProcessing: false,
            errorMessage: '',
...
<div class="min-h-screen bg-gradient-to-b from-indigo-50 to-blue-100 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden" x-data="paymentForm">
            <form x-ref="paymentForm" method="POST" action="{{ route('paiement.initialiser', ['facturationId' => $facturation_id ?? '']) }}">
                @csrf
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-100">
                            <div class="space-y-3">
                                <div class="flex justify-between">
                                    ...
                                    <span class="font-medium text-gray-800">{{ $subscription['subscription_plan'] }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Entreprise</span>
                                    <span class="font-medium text-gray-800">{{ $company['company_name'] }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Nombre d'utilisateurs</span>
                                    <span class="font-medium text-gray-800">{{ $company['company_size'] }}</span>
                                </div>
                                <div class="border-t border-gray-200 my-2"></div>
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Coût fixe</span>
                                    <span class="font-medium text-gray-800" x-text="formatNumber({{ $subscription['base_cost'] }}) + ' FCFA'"></span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Coût par utilisateur</span>
                                    <span class="font-medium text-gray-800" x-text="formatNumber({{ $subscription['user_cost'] }}) + ' FCFA'"></span>
                                </div>
                                <div class="border-t border-gray-200 my-2"></div>
                                <div class="flex justify-between text-lg font-bold">
                                    <span class="text-gray-800">Total mensuel</span>
                                    <span class="text-indigo-600" x-text="formatNumber({{ $subscription['total_cost'] }}) + ' FCFA'"></span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-gray-50 px-6 py-4 sm:px-8 border-t border-gray-200 mt-8">
                    <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                        <a 
                            href="{{ route('workflow.subscription') }}" 
