...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Gestion des filiales</h1>
        <a href="{{ route('admin.filiales.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg flex items-center">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($filiales as $filiale)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $filiale->code }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $filiale->nom }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $filiale->ville }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $filiale->pays }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    @if($filiale->getResponsablePrincipalActuel())
                                        {{ $filiale->getResponsablePrincipalActuel()->nom }} {{ $filiale->getResponsablePrincipalActuel()->prenom }}
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('admin.filiales.show', $filiale) }}" class="text-indigo-600 hover:text-indigo-900" title="Voir">
                                        ...
                                        <a href="{{ route('admin.filiales.edit', $filiale) }}" class="text-blue-600 hover:text-blue-900" title="Modifier">
                                        ...
                                        <form action="{{ route('admin.filiales.destroy', $filiale) }}" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette filiale?');">
                                            @csrf
                                            @method('DELETE')
        ...
        @else
            <div class="p-6 text-center text-gray-500">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto text-gray-400 mb-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                <p class="text-lg">Aucune filiale n'a été trouvée.</p>
                <p class="mt-2">Commencez par ajouter une nouvelle filiale pour votre entreprise.</p>
            </div>
        @endif
    </div>
</div>
