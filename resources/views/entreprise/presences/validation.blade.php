...
<div class="container-fluid px-4">
    ...
    <ol class="breadcrumb mb-4">
        ...
        <li class="breadcrumb-item active">Validation des pointages</li>
    </ol>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-striped" id="validationTable">
                    <thead>
                        <tr>
                            <th>Date</th>
                            <th>Employé</th>
                    ...
                    <tbody>
                        @forelse($presencesEnAttente as $presence)
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-info mb-1" data-bs-toggle="modal" data-bs-target="#detailsModal{{ $presence->id }}">
                                        <i class="fas fa-eye"></i> Détails
                                    </button>
                                    <div class="btn-group mb-1">
                                        <button type="button" class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#validerModal{{ $presence->id }}">
                                            <i class="fas fa-check"></i> Valider
                                        </button>
                                        <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#rejeterModal{{ $presence->id }}">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    ...
                                                    @if($presence->latitude && $presence->longitude)
                                                        ...
                                                        @endif
                                                    @endif
                                                    @if($presence->verification_data && isset($presence->verification_data['hors_site']) && $presence->verification_data['hors_site'])
                                                        <div class="alert alert-warning">
                                                            <p><strong>Attention:</strong> Pointage effectué hors du périmètre du site</p>
                                                            <p>Distance: {{ round($presence->verification_data['distance'], 2) }} m (Rayon autorisé: {{ $presence->verification_data['rayon_autorise'] }} m)</p>
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
                            ...
                            <!-- Modal de validation -->
                            <div class="modal fade" id="validerModal{{ $presence->id }}" tabindex="-1" aria-labelledby="validerModalLabel{{ $presence->id }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="validerModalLabel{{ $presence->id }}">Valider ce pointage</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <form action="{{ route('entreprise.presences.valider', $presence->id) }}" method="POST">
                                            @csrf
                                            <div class="modal-body">
                                                <p>Êtes-vous sûr de vouloir valider ce pointage ?</p>
                                                <p><strong>Date:</strong> {{ $presence->created_at->format('d/m/Y H:i') }}</p>
                                                <p><strong>Employé:</strong> 
                                                    @if($presence->user)
                                                        {{ $presence->user->nom }} {{ $presence->user->prenom }}
                                                    @else
                            ...
                            <!-- Modal de rejet -->
                            <div class="modal fade" id="rejeterModal{{ $presence->id }}" tabindex="-1" aria-labelledby="rejeterModalLabel{{ $presence->id }}" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="rejeterModalLabel{{ $presence->id }}">Rejeter ce pointage</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <form action="{{ route('entreprise.presences.rejeter', $presence->id) }}" method="POST">
                                            @csrf
                                            <div class="modal-body">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center">Aucun pointage en attente de validation</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
